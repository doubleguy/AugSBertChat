...
def get_predict_like_probability_file(all_pred):
    ...
    with open('./datasets/gold_dev.txt', 'r') as f1, open('./predict/predict_eval.txt', 'w') as f2:
        for line in f1:
...
def do_test(config):
    ...
    test_dataset = MyDataset(config.eval_data_path)
    test_dataloader = DataLoader(test_dataset, batch_size=config.per_device_eval_batch_size)
